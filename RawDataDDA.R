...
# Input data
raw<-read.table(file = "RawData.DDA.txt", header = TRUE, sep = ",")
head(raw)
...
##                   need to make the parameter adjustments


matrix(c(p.adjust(resultMultiComparisons$ComparisonResult$pvalue,method = "BH",length(resultMultiComparisons$ComparisonResult$pvalue))),ncol = 1)
###############################################
### need to set parameters to keep cases with, |log2FC| > 2, and qvalue < 0.05
########  boolean statments: 
########   abs(resultOneComparison$ComparisonResult$log2FC) > 1
########   resultOneComparison$ComparisonResult$adj.pvalue < 0.05
#####################################
